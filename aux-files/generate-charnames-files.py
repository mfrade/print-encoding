...
for j, r in enumerate(ranges):
    ...
    char_filename = base_char_filename + "{:03d}".format(r.order) + ".txt"

    if not os.path.exists(char_filename):
        with open(char_filename, "w") as file:
            print(f"Starting to extract char names to {char_filename}")
        ...
        print(f'{char_filename} already exists.')
